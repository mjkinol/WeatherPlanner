...
public class AerisSimpleForecastSource implements IForecastSource {
	...
	public WeatherLocation getCurrentWeather(String loc, TempUnit unit) {
		String loca = loc;
		loca.trim();
		String url = locationsUrl;
		if(isNumeric(loca)) {
			url = locationsUrl + String.format("&p=%s", loca);
		}
		else {
			loca = loca.replaceAll(" ", "%20");
			url = locationsUrl + String.format("&p=%s", loca);
		}
		...
		for(int i=0; i<city.length(); i++) {
			...
			else if(city.charAt(i) == ' ') {
				city = city.substring(0,i+1) + city.substring(i+1,i+2).toUpperCase() + city.substring(i+2);
			}
		}
		String country = place.get("country").getAsString().toUpperCase();
	    ...
	    return new WeatherLocation(city, country, weatherDisc, icon.getStaticPath(), 
	    		icon.getAnimatePath(), avgLow, avgHigh, currentTemp, unit, 0, false, 0);
	}
