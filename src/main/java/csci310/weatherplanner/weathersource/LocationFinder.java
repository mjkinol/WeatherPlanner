...
public class LocationFinder implements ILocationFinder{
	...
	public WeatherFilter getLocations(String location,String activity, int maxResults, int radius, TempUnit unit) {
		...
		if(unit == TempUnit.Celsius) {
			double tmin = (tempMin-32)/1.8;
			double tmax = (tempMax-32)/1.8;
			tempMin = (int) Math.round(tmin);
			tempMax = (int) Math.round(tmax);
		}
		String loc = location;
		...
		WeatherFilter wf = new WeatherFilter(loc,tempMax, tempMin, precipType, max, rad);
		return wf;
	}
