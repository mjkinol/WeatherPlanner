...
public class AerisAdvancedWeatherSource implements IAdvancedWeatherSource {
	private final static String clientId = "BHgBwkZbkIhh4cQCJk3yb";
	private final static String clientSecret = "075WDiZdjHv1uJ41kGlVIJNmeqzIPu7B888quXRo";
	...
	public List<WeatherLocation> getLocations(WeatherFilter filter, TempUnit unit) {
		...
		for(JsonElement element: locations) {
			...
			if(period != null) {
				if(!period.get("temp").isJsonNull()) {
					temp = (JsonObject) period.get("temp");
				}
			}
			...
			try {
				...
				int location_id = likesManager.getLocationId(normalLoc);
				if (location_id != -1) {
					num_likes = likesManager.getNumberOfLikes(location_id);
				}
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}

			result.add(new WeatherLocation(city, country, weatherDisc, icon.getStaticPath(), icon.getAnimatePath(), avgLow, avgHigh, currentTemp, unit, distance, false, num_likes));
	...
	private String createLocationsUrl(WeatherFilter filter, TempUnit unit) {
		...
		params.append(String.format("&radius=%d", filter.getRadius()));
		...
		case Rainy:
			params.append("&filter=wxrain,metar");
			break;
		case Snowing:
			params.append("&filter=wxsnow,metar");
			break;
		case Storming:
			params.append("&filter=wxrain,metar");
			break;
		default:
			params.append("&filter=metar");
			break;
		}
