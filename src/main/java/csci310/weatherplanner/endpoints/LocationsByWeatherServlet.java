...
import csci310.weatherplanner.weathersource.PrecipType;
...
public class LocationsByWeatherServlet extends HttpServlet {
    ...
    public LocationsByWeatherServlet(WeatherSource weatherSource) {
    	super();
    	gson = new Gson();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(
				...
				!request.getParameterMap().containsKey("max") ||
				!request.getParameterMap().containsKey("min") ||
		...
		PrecipType precipType = Enums.getIfPresent(PrecipType.class, request.getParameter("precip")).orNull();
		if(precipType == null || unit == null)
		{
			response.setStatus(400);
		}
		
		
		WeatherFilter filter = new WeatherFilter(loc, tempMax, tempMin, precipType, maxResults, radius);
